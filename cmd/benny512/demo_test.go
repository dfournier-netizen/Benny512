...
func TestDemoLogArmedBeforeStartCapturesSeedTraffic(t *testing.T) {
	...
	// Windows does not allow TempDir cleanup to unlink an open log file. The
	// production process closes the server on shutdown; this direct buildDemo
	// test must do the same rather than leaving its configured log open.
	defer srv.Close()
